_G["db"]["Units"]["Cars"]["Car"]["#Index"] = {
	CanopyGeometry = {
		azimuth = {},
		elevation = {}
	},
	DetectionRange = 35000,
	DisplayName = 'MCC-SR Sborka "Dog Ear" SR',
	DisplayNameShort = "Sborka",
	IR_emission_coeff = 0.08,
	MaxSpeed = 60.00012,
	Name = 'MCC-SR Sborka "Dog Ear" SR',
	Rate = 15,
	Sensors = {
		OPTIC = { "generic tank daysight" },
		RADAR = "Dog Ear radar"
	},
	ThreatRange = 0,
	WS = { {
			LN = { {
					ECM_K = 0.65,
					distanceMax = 35000,
					distanceMin = 100,
					max_trg_alt = 10000,
					min_trg_alt = 15,
					reactionTime = 12,
					reflection_limit = 0.18,
					type = 101
				} },
			angles = { { 3.1415926535898, -3.1415926535898, 0, 0.69813170079773 } },
			omegaY = 1,
			omegaZ = 1,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					ECM_K = 0.65,
					distanceMax = 35000,
					distanceMin = 100,
					max_trg_alt = 10000,
					min_trg_alt = 15,
					reactionTime = 12,
					reflection_limit = 0.18,
					type = 101
				} },
			angles = { { 3.1415926535898, -3.1415926535898, 0, 0.69813170079773 } },
			omegaY = 1,
			omegaZ = 1,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					ECM_K = 0.65,
					distanceMax = 35000,
					distanceMin = 100,
					max_trg_alt = 10000,
					min_trg_alt = 15,
					reactionTime = 12,
					reflection_limit = 0.18,
					type = 101
				} },
			angles = { { 3.1415926535898, -3.1415926535898, 0, 0.69813170079773 } },
			omegaY = 1,
			omegaZ = 1,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					ECM_K = 0.65,
					distanceMax = 35000,
					distanceMin = 100,
					max_trg_alt = 10000,
					min_trg_alt = 15,
					reactionTime = 12,
					reflection_limit = 0.18,
					type = 101
				} },
			angles = { { 3.1415926535898, -3.1415926535898, 0, 0.69813170079773 } },
			omegaY = 1,
			omegaZ = 1,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					ECM_K = 0.65,
					distanceMax = 35000,
					distanceMin = 100,
					max_trg_alt = 10000,
					min_trg_alt = 15,
					reactionTime = 12,
					reflection_limit = 0.18,
					type = 101
				} },
			angles = { { 3.1415926535898, -3.1415926535898, 0, 0.69813170079773 } },
			omegaY = 1,
			omegaZ = 1,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					ECM_K = 0.65,
					distanceMax = 35000,
					distanceMin = 100,
					max_trg_alt = 10000,
					min_trg_alt = 15,
					reactionTime = 12,
					reflection_limit = 0.18,
					type = 101
				} },
			angles = { { 3.1415926535898, -3.1415926535898, 0, 0.69813170079773 } },
			omegaY = 1,
			omegaZ = 1,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					ECM_K = 0.65,
					distanceMax = 35000,
					distanceMin = 100,
					max_trg_alt = 10000,
					min_trg_alt = 15,
					reactionTime = 12,
					reflection_limit = 0.18,
					type = 101
				} },
			angles = { { 3.1415926535898, -3.1415926535898, 0, 0.69813170079773 } },
			omegaY = 1,
			omegaZ = 1,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					ECM_K = 0.65,
					distanceMax = 35000,
					distanceMin = 100,
					max_trg_alt = 10000,
					min_trg_alt = 15,
					reactionTime = 12,
					reflection_limit = 0.18,
					type = 101
				} },
			angles = { { 3.1415926535898, -3.1415926535898, 0, 0.69813170079773 } },
			omegaY = 1,
			omegaZ = 1,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					ECM_K = 0.65,
					distanceMax = 35000,
					distanceMin = 100,
					max_trg_alt = 10000,
					min_trg_alt = 15,
					reactionTime = 12,
					reflection_limit = 0.18,
					type = 101
				} },
			angles = { { 3.1415926535898, -3.1415926535898, 0, 0.69813170079773 } },
			omegaY = 1,
			omegaZ = 1,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					ECM_K = 0.65,
					distanceMax = 35000,
					distanceMin = 100,
					max_trg_alt = 10000,
					min_trg_alt = 15,
					reactionTime = 12,
					reflection_limit = 0.18,
					type = 101
				} },
			angles = { { 3.1415926535898, -3.1415926535898, 0, 0.69813170079773 } },
			omegaY = 1,
			omegaZ = 1,
			pos = { 0, 3, 0 }
		},
		isDetector = true,
		maxTargetDetectionRange = 35000,
		radar_rotation_type = 0,
		radar_type = 104,
		searchRadarFrequencies = { { 3000000000, 6000000000 } }
	},
	Waypoint_Custom_Panel = true,
	animation_arguments = {
		crew_presence = 50,
		locator_rotation = 11
	},
	armour_scheme = {
		hull_azimuth = { {} },
		hull_elevation = { {} },
		turret_azimuth = { {} },
		turret_elevation = { {} }
	},
	attribute = { 2, 16, 101, "Redacted", "SAM SR", "RADAR_BAND1_FOR_ARM", "RADAR_BAND2_FOR_ARM", "All", "Ground Units", "Vehicles", "Ground vehicles", "NonAndLightArmoredUnits", "NonArmoredUnits", "Air Defence", "SAM related", "SAM elements" },
	category = "Air Defence",
	chassis = {},
	enablePlayerCanDrive = false,
	mapclasskey = "P0091000046",
	radar_rotation_period = 3,
	sensor = {
		height = 3.8,
		max_alt_finding_target = 10000,
		max_range_finding_target = 35000,
		min_range_finding_target = 100
	},
	snd = {
		engine_pitch = { {}, {}, {} },
		engine_vol_a = { {}, {}, {} },
		engine_vol_v = { {}, {}, {} },
		move_pitch = { {}, {} },
		move_vol = { {}, {}, {} },
		radarRotation = "GndTech/RadarRotation"
	},
	swapped_names = true,
	swing_on_run = false,
	tags = { "Air Defence", "Search Radar" },
	type = "Dog Ear radar",
	visual = {
		IR = {
			coeffs = { { 0.035, 0.0012 }, { 0.0069, 0.0036, 4.8 }, { 0.205, 0.005, 2 }, { 0.01, 0.0017 }, { 0.2, 0.0002 } }
		},
		agony_explosion_size = 2,
		dirt_pos = { -2.8, 0.4, -1.128 },
		dust_pos = { 2.7, 0, -1.128 },
		fire_pos = { 0.2, 0.8, 0 },
		fire_size = 0.6,
		fire_time = 300,
		max_time_agony = 90,
		min_time_agony = 5,
		shape = "Sborka",
		shape_dstr = "Sborka_P_1"
	}
}