_G["db"]["Units"]["Cars"]["Car"]["#Index"] = {
	CustomAimPoint = { 0, 1.5, 0 },
	DetectionRange = 0,
	DisplayName = 'SAM SA-10 S-300 "Grumble" C2 ',
	DisplayNameShort = "SA-10 CP",
	IR_emission_coeff = 0.05,
	MaxSpeed = 60.00012,
	Name = 'SAM SA-10 S-300 "Grumble" C2 ',
	Rate = 20,
	Sensors = {
		RADAR = { "S-300PS 40B6M tr" }
	},
	ThreatRange = 0,
	WS = { {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		}, {
			LN = { {
					depends_on_unit = { { { "S-300PS 40B6MD sr" } }, { { "S-300PS 64H6E sr" } } },
					distanceMax = 160000,
					distanceMin = 2000,
					max_trg_alt = 27000,
					min_trg_alt = 5,
					reactionTime = 0.1,
					reflection_limit = 0.049,
					type = 100
				} },
			angles = { { 3.1415926535898, -3.1415926535898, -1.5707963267949, 1.5707963267949 } },
			omegaY = 3,
			omegaZ = 3,
			pos = { 0, 3, 0 }
		},
		maxTargetDetectionRange = 160000
	},
	Waypoint_Custom_Panel = true,
	animation_arguments = {},
	armour_scheme = {
		hull_azimuth = { {} },
		hull_elevation = { {} },
		turret_azimuth = { {} },
		turret_elevation = { {} }
	},
	attribute = { 2, 16, 25, "Redacted", "SAM CC", "CustomAimPoint", "All", "Ground Units", "Vehicles", "Ground vehicles", "NonAndLightArmoredUnits", "NonArmoredUnits", "Air Defence", "SAM related", "SAM elements" },
	category = "Air Defence",
	chassis = {},
	enablePlayerCanDrive = false,
	mapclasskey = "P0091000046",
	sensor = {
		height = 8.68,
		max_alt_finding_target = 27000,
		max_range_finding_target = 160000,
		min_range_finding_target = 2000
	},
	snd = {},
	swapped_names = true,
	swing_on_run = false,
	tags = { "Air Defence", "Command & Control" },
	type = "S-300PS 54K6 cp",
	visual = {
		IR = {
			coeffs = { { 0.031, 0.0022 }, { 0.0069, 0.0036 }, { 0, 0 }, { 0, 0 }, { 0.2, 0.00036363636363636 } }
		},
		agony_explosion_size = 2,
		fire_pos = { 0.9, 1.1, 0 },
		fire_size = 0.6,
		fire_time = 500,
		max_time_agony = 80,
		min_time_agony = 10,
		shape = "54k6",
		shape_dstr = "54k6_p_1"
	}
}