_G["db"]["Units"]["Ships"]["Ship"]["#Index"] = {
	ArrestingGears = { {
			Left = {
				pos = { -68.54, 12, -8.869 }
			},
			Right = {
				pos = { -65.673, 12, 10.473 }
			}
		}, {
			Left = {
				pos = { -62.886, 12, -8.998 }
			},
			Right = {
				pos = { -59.99, 12, 10.575 }
			}
		}, {
			Left = {
				pos = { -57.739, 12, -9.508 }
			},
			Right = {
				pos = { -54.841, 12, 9.971 }
			}
		}, {
			Left = {
				pos = { -51.021, 12, -10.536 }
			},
			Right = {
				pos = { -48.026, 12, 9.964 }
			}
		}, {
			Left = {
				pos = { -45.085, 12, -10.589 }
			},
			Right = {
				pos = { -42.157, 12, 9.184 }
			}
		}, {
			Left = {
				pos = { -39.065, 12, -10.589 }
			},
			Right = {
				pos = { -36.087, 12, 9.616 }
			}
		},
		ArrestingGearsNumber = 6
	},
	Categories = { {
			name = "AircraftCarrier"
		}, {
			name = "AircraftCarrier With Catapult"
		}, {
			name = "Armed Ship"
		}, {
			name = "AircraftCarrier With Arresting Gear"
		} },
	DM = { {
			area_arg = 70,
			area_fire = {
				pos = { 30, 14.5, 12.6 },
				size = 0.5
			},
			area_life = 50,
			area_name = "ISLAND"
		}, {
			area_arg = 71,
			area_fire = {
				pos = { 75.5, 11.1, 5.1 },
				size = 0.5
			},
			area_life = 200,
			area_name = "HULL_FWD_PORT"
		}, {
			area_arg = 72,
			area_fire = {
				pos = { 14.5, -12.5, 5.1 },
				size = 0.5
			},
			area_life = 200,
			area_name = "HULL_MID_PORT"
		}, {
			area_arg = 73,
			area_fire = {
				pos = { -58, -11.3, 5.1 },
				size = 0.5
			},
			area_life = 200,
			area_name = "HULL_AFT_PORT"
		}, {
			area_arg = 76,
			area_fire = {
				pos = { 74.5, 11.1, 5.1 },
				size = 0.5
			},
			area_life = 200,
			area_name = "HULL_FWD_STBD"
		}, {
			area_arg = 77,
			area_fire = {
				pos = { 10.5, -12.5, 5.1 },
				size = 0.5
			},
			area_life = 200,
			area_name = "HULL_MID_STBD"
		}, {
			area_arg = 78,
			area_fire = {
				pos = { -52, 11.3, 5.1 },
				size = 0.5
			},
			area_life = 200,
			area_name = "HULL_AFT_STBD"
		}, {
			area_arg = 79,
			area_fire = {
				pos = { 87.5, 0, 10 },
				size = 0.5
			},
			area_life = 200,
			area_name = "PALUBA_FWD",
			belongsToRunway = true
		}, {
			area_arg = 80,
			area_fire = {
				pos = { 17.2, -3.7, 10 },
				size = 0.5
			},
			area_life = 200,
			area_name = "PALUBA_MID",
			belongsToRunway = true
		}, {
			area_arg = 81,
			area_fire = {
				pos = { -66.1, 1.9, 10 },
				size = 0.5
			},
			area_life = 200,
			area_name = "PALUBA_AFT",
			belongsToRunway = true
		}, {
			area_arg = 82,
			area_life = 50,
			area_name = "ISLAND-FUNNEL"
		}, {
			area_arg = 83,
			area_fire = {
				pos = { -38.2, 0, 10 },
				size = 0.5
			},
			area_life = 100,
			area_name = "PALUBA-LIFT-AFT",
			belongsToRunway = true
		}, {
			area_arg = 84,
			area_fire = {
				pos = { 60.8, 0, 10 },
				size = 0.5
			},
			area_life = 100,
			area_name = "PALUBA-LIFT-FWD"
		} },
	DeckLevel = 11.987,
	DetectionRange = 18000,
	DisplayName = "ARA Veinticinco de Mayo",
	Gamma_max = 0.35,
	Height = 35.226,
	HelicopterSpawnTerminal = { {
			Points = { { { -76.074, 11.987, 0 }, 0 } },
			TerminalIdx = 1
		}, {
			Points = { { { -14.891, 11.987, -7.568 }, 0 } },
			TerminalIdx = 2
		}, {
			Points = { { { 72.633, 11.987, -2.068 }, 0 } },
			TerminalIdx = 3
		}, {
			Points = { { { -51.013, 11.987, 3, 175 }, 0 } },
			TerminalIdx = 4
		}, {
			Points = { { { 14.75, 11.987, -6.607 }, 0 } },
			TerminalIdx = 5
		}, {
			Points = { { { 96.349, 11.987, 0.48 }, 0 } },
			TerminalIdx = 6
		},
		TerminalNumber = 6
	},
	Helicopter_Num_ = 6,
	ICLS = true,
	ICLS_Glideslope_position = { -57.387, 14.3, 18, 4.5 },
	ICLS_Localizer_position = { -96.19, 6.501, 10.77, 188 },
	LSOView = {
		cockpit = "empty",
		position = {
			offset = { 0, 0, 0, 0, 0 }
		}
	},
	Landing_Point = { -57.409, 12, 1.046 },
	Length = 236,
	MaxSpeed = 44.64,
	Name = "ARA Veinticinco de Mayo",
	OLS = {
		CutLightsArg = 404,
		DatumAndWaveOffLightsArg = 405,
		GlideslopeBasicAngle = 4.5,
		MeatBallArg = 151,
		Type = 0,
		VerticalCoverageAngle = 1.7
	},
	Om = 0.05,
	Plane_Num_ = 15,
	RCS = 700000,
	R_min = 665.8,
	Rate = 5500,
	RunWays = { { { -8.435, 11.987, -6.5 }, 351.8, 170, 28, 0, 3.5, 4, 4.5, 5, 5.5, 6 }, { { 55.621, 11.987, -8.41 }, 4.41, 66, 25, 0, 2.5, 2.8, 3, 3, 3.2, 3.5 },
		RunwaysNumber = 2
	},
	Sensors = {
		OPTIC = { "long-range naval optics", "long-range naval LLTV", "long-range naval FLIR" },
		RADAR = { "Osa 9A33 ln", "rezki search radar", "carrier search radar" }
	},
	Singleton = "yes",
	Stations = {
		LSO = {
			HQ_model = "empty",
			cameraPos = { -76.73, 13.506, -11.9, 188 },
			dockConnector = "POINT_KATAPULT_01",
			hide_argument = 821
		}
	},
	TACAN = true,
	TACAN_position = { 31.1, 49.32, 12.76 },
	Tail_Width = 10,
	TaxiForTORoutes = { {
			Points = { { { 3.831, 11.987, 11.525 }, 1 }, { { 4.987, 11.987, 5.445 }, 1 }, { { 15.376, 11.987, -12.069 }, 1 }, { { 20.634, 11.987, -11.65 }, 1 } },
			RunwayIdx = 1
		}, {
			Points = { { { -9.576, 11.987, 11.154 }, 1 }, { { -4.992, 11.987, 4.685 }, 1 }, { { 4.987, 11.987, 5.445 }, 1 }, { { 15.376, 11.987, -12.069 }, 1 }, { { 20.634, 11.987, -11.65 }, 1 } },
			RunwayIdx = 1
		}, {
			Points = { { { -2.195, 11.987, -11.932 }, 1 }, { { 0.921, 11.987, -7.89 }, 1 }, { { 7.255, 11.987, -5.437 }, 1 }, { { 10.631, 11.987, -9.772 }, 1 }, { { 15.376, 11.987, -12.069 }, 1 }, { { 20.634, 11.987, -11.65 }, 1 } },
			RunwayIdx = 1
		}, {
			Points = { { { -15.032, 11.987, -10.423 }, 1 }, { { -11.38, 11.987, -5.745 }, 1 }, { { 0.921, 11.987, -7.89 }, 1 }, { { 15.376, 11.987, -12.069 }, 1 }, { { 20.634, 11.987, -11.65 }, 1 } },
			RunwayIdx = 1
		}, {
			Points = { { { -28.628, 11.987, -9.389 }, 1 }, { { -24.105, 11.987, -3.815 }, 1 }, { { -4.992, 11.987, 4.685 }, 1 }, { { 4.987, 11.987, 5.445 }, 1 }, { { 15.376, 11.987, -12.069 }, 1 }, { { 20.634, 11.987, -11.65 }, 1 } },
			RunwayIdx = 1
		}, {
			Points = { { { -42.022, 11.987, -8.061 }, 1 }, { { -37.09, 11.987, -1.939 }, 1.5 }, { { -4.992, 11.987, 4.685 }, 1 }, { { 4.987, 11.987, 5.445 }, 1 }, { { 15.376, 11.987, -12.069 }, 1 }, { { 20.634, 11.987, -11.65 }, 1 } },
			RunwayIdx = 1
		}, {
			Points = { { { -55.454, 11.987, -6.095 }, 1 }, { { -50.57, 11.987, -0.001 }, 1.5 }, { { 4.987, 11.987, 5.445 }, 1 }, { { 15.376, 11.987, -12.069 }, 1 }, { { 20.634, 11.987, -11.65 }, 1 } },
			RunwayIdx = 1
		}, {
			Points = { { { -72.392, 11.987, 5.292 }, 1 }, { { -63.326, 11.987, 4.024 }, 2 }, { { 4.987, 11.987, 5.445 }, 2 }, { { 15.376, 11.987, -12.069 }, 2 }, { { 20.634, 11.987, -11.65 }, 2 } },
			RunwayIdx = 1
		},
		RoutesForTONumber = 8
	},
	TaxiRoutes = { { { { 50, 11.987, -14.84 }, 4 }, { { 53.022, 11.987, 1.05 }, 2 }, { { 49.651, 11.987, 4.047 }, 3 }, { { 38.672, 11.987, 4.047 }, 3 }, { { -2, 11.987, 4.047 }, 3 }, { { 4, 11.987, 2.9 }, 2 }, { { -11.576, 11.987, 13.2 }, -2, 60 } }, { { { 50, 11.987, -14.84 }, 4 }, { { 53.022, 11.987, 1.05 }, 2 }, { { 49.651, 11.987, 4.047 }, 3 }, { { 38.672, 11.987, 4.047 }, 3 }, { { 6, 11.987, 4 }, 3 }, { { 8, 11.987, 1.4 }, 2 }, { { 2.5, 11.987, 13.8 }, -2, 60 } }, { { { 50, 11.987, -14.84 }, 4 }, { { 53.152, 11.987, -4.84 }, 2 }, { { 49.651, 11.987, 4.047 }, 3 }, { { 55.2, 11.987, -3 }, 2 }, { { 55, 11.987, 5 }, -2 }, { { 55, 11.987, 16.15 }, -2, 60 } }, { { { 50, 11.987, -14.84 }, 4 }, { { 75, 11.987, 8.743 }, 2 }, { { 80, 11.987, 9 }, 2 }, { { 87.2, 11.987, 9.6 }, 2, 60 } }, { { { 50, 11.987, -14.84 }, 4 }, { { 75, 11.987, 8.743 }, 2 }, { { 77, 11.987, 10.5 }, 2, 60 } },
		RoutesNumber = 5
	},
	ThreatRange = 5000,
	WS = { {
			LN = { {
					ECM_K = 0.65,
					beamWidth = 1.5707963267949,
					distanceMax = 18000,
					distanceMin = 400,
					frequencyRange = { 500000000, 580000000 },
					max_number_of_missiles_channels = 1,
					max_trg_alt = 25000,
					min_trg_alt = 5,
					reactionTime = 2.5,
					reflection_limit = 0.1,
					type = 102
				} },
			angles = { { 3.1415926535898, 1.5707963267949, -0.17453292519943, 1.3962634015955 } },
			area = "ISLAND",
			center = "POINT_RADAR_LW02-LOWER",
			omegaY = 1,
			omegaZ = 1,
			pidY = {
				d = 12,
				i = 0.05,
				inn = 50,
				p = 100
			},
			pidZ = {
				d = 12,
				i = 0.05,
				inn = 50,
				p = 100
			},
			reference_angle_Y = 1.5707963267949
		}, {
			LN = { {
					frequencyRange = {}
				} },
			angles = { { 1.5707963267949, -2.9670597283904 } },
			area = "ISLAND",
			center = "POINT_RADAR_LW02-LOWER",
			pidY = {},
			pidZ = {},
			reference_angle_Y = 1.5707963267949
		}, {
			LN = { {
					BR = { {
							connector_name = "POINT_BOFORS_1_1",
							recoilArgument = 121,
							recoilTime = 0.47
						} },
					PL = { {
							shell_name = {}
						} },
					sensor = {},
					sightIndicationMode = 0,
					sightMasterMode = 1
				} },
			angles = { { 0, -2.7925268031909, -0.087266462599716, 1.221730476396 } },
			area = "HULL_FWD_STBD",
			center = "CENTER_TOWER_01",
			cockpit = { "genericAAA", { -3.57, 0.304, 0.596 },
				open = true
			},
			drawArgument1 = 41,
			drawArgument2 = 51,
			pointer = "POINT_BOFORS_1_1",
			reference_angle_Y = -1.5707963267949,
			reference_angle_Z = 0
		}, {
			LN = { {
					BR = { {
							connector_name = "POINT_BOFORS_2_1",
							recoilArgument = 122,
							recoilTime = 0.47
						} },
					PL = { {
							shell_name = {}
						} },
					sensor = {},
					sightIndicationMode = 0,
					sightMasterMode = 1
				} },
			angles = { { -0.34906585039887, -2.7925268031909, -0.087266462599716, 1.221730476396 } },
			area = "HULL_FWD_STBD",
			center = "CENTER_TOWER_02",
			cockpit = { "genericAAA", { -3.57, 0.304, 0.596 },
				open = true
			},
			drawArgument1 = 42,
			drawArgument2 = 52,
			pointer = "POINT_BOFORS_2_1",
			reference_angle_Y = -1.5707963267949,
			reference_angle_Z = 0
		}, {
			LN = { {
					BR = { {
							connector_name = "POINT_BOFORS_3_1",
							recoilArgument = 123,
							recoilTime = 0.47
						} },
					PL = { {
							shell_name = {}
						} },
					sensor = {},
					sightIndicationMode = 0,
					sightMasterMode = 1
				} },
			angles = { { -0.34906585039887, -2.6179938779915, -0.087266462599716, 1.221730476396 } },
			area = "HULL_AFT_STBD",
			center = "CENTER_TOWER_03",
			cockpit = { "genericAAA", { -3.57, 0.304, 0.596 },
				open = true
			},
			drawArgument1 = 43,
			drawArgument2 = 53,
			pointer = "POINT_BOFORS_3_1",
			reference_angle_Y = -1.5707963267949,
			reference_angle_Z = 0
		}, {
			LN = { {
					BR = { {
							connector_name = "POINT_BOFORS_4_1",
							recoilArgument = 124,
							recoilTime = 0.47
						} },
					PL = { {
							shell_name = {}
						} },
					sensor = {},
					sightIndicationMode = 0,
					sightMasterMode = 1
				} },
			angles = { { -0.34906585039887, -2.9670597283904, -0.087266462599716, 1.221730476396 } },
			area = "HULL_AFT_STBD",
			center = "CENTER_TOWER_04",
			cockpit = { "genericAAA", { -3.57, 0.304, 0.596 },
				open = true
			},
			drawArgument1 = 44,
			drawArgument2 = 54,
			pointer = "POINT_BOFORS_4_1",
			reference_angle_Y = -1.5707963267949,
			reference_angle_Z = 0
		}, {
			LN = { {
					BR = { {
							connector_name = "POINT_BOFORS_5_1",
							recoilArgument = 125,
							recoilTime = 0.47
						} },
					PL = { {
							shell_name = {}
						} },
					sensor = {},
					sightIndicationMode = 0,
					sightMasterMode = 1
				} },
			angles = { { 2.9670597283904, 0.26179938779915, -0.087266462599716, 1.221730476396 } },
			area = "HULL_AFT_PORT",
			center = "CENTER_TOWER_05",
			cockpit = { "genericAAA", { -3.57, 0.304, 0.596 },
				open = true
			},
			drawArgument1 = 45,
			drawArgument2 = 55,
			pointer = "POINT_BOFORS_5_1",
			reference_angle_Y = 1.5707963267949,
			reference_angle_Z = 0
		}, {
			LN = { {
					BR = { {
							connector_name = "POINT_BOFORS_6_1",
							recoilArgument = 126,
							recoilTime = 0.47
						} },
					PL = { {
							shell_name = {}
						} },
					sensor = {},
					sightIndicationMode = 0,
					sightMasterMode = 1
				} },
			angles = { { 2.6179938779915, 0.26179938779915, -0.087266462599716, 1.221730476396 } },
			area = "HULL_AFT_PORT",
			center = "CENTER_TOWER_06",
			cockpit = { "genericAAA", { -3.57, 0.304, 0.596 },
				open = true
			},
			drawArgument1 = 46,
			drawArgument2 = 56,
			pointer = "POINT_BOFORS_6_1",
			reference_angle_Y = 1.5707963267949,
			reference_angle_Z = 0
		}, {
			LN = { {
					BR = { {
							connector_name = "POINT_BOFORS_7_1",
							recoilArgument = 127,
							recoilTime = 0.47
						} },
					PL = { {
							shell_name = {}
						} },
					sensor = {},
					sightIndicationMode = 0,
					sightMasterMode = 1
				} },
			angles = { { 1.7453292519943, 0.26179938779915, -0.087266462599716, 1.221730476396 } },
			area = "HULL_FWD_PORT",
			center = "CENTER_TOWER_07",
			cockpit = { "genericAAA", { -3.57, 0.304, 0.596 },
				open = true
			},
			drawArgument1 = 47,
			drawArgument2 = 57,
			pointer = "POINT_BOFORS_7_1",
			reference_angle_Y = 1.5707963267949,
			reference_angle_Z = 0
		}, {
			LN = { {
					BR = { {
							connector_name = "POINT_BOFORS_8_1",
							recoilArgument = 128,
							recoilTime = 0.47
						} },
					PL = { {
							shell_name = {}
						} },
					sensor = {},
					sightIndicationMode = 0,
					sightMasterMode = 1
				} },
			angles = { { 1.9198621771938, 0, -0.087266462599716, 1.221730476396 } },
			area = "HULL_FWD_PORT",
			center = "CENTER_TOWER_08",
			cockpit = { "genericAAA", { -3.57, 0.304, 0.596 },
				open = true
			},
			drawArgument1 = 48,
			drawArgument2 = 58,
			pointer = "POINT_BOFORS_8_1",
			reference_angle_Y = 1.5707963267949,
			reference_angle_Z = 0
		}, {
			LN = { {
					BR = { {
							connectorFire = true,
							connector_name = "POINT_BOFORS_9_1",
							recoilArgument = 129,
							recoilTime = 0.47
						} },
					PL = { {
							shell_name = {}
						} },
					sensor = {},
					sightIndicationMode = 0,
					sightMasterMode = 1
				} },
			angles = { { -1.5707963267949, 1.5707963267949, -0.087266462599716, 1.221730476396 } },
			area = "ISLAND",
			center = "CENTER_TOWER_09",
			cockpit = { "genericAAA", { -3.57, 0.304, 0.596 },
				open = true
			},
			drawArgument1 = 49,
			drawArgument2 = 59,
			pointer = "POINT_BOFORS_9_1",
			reference_angle_Y = 3.1415926535898,
			reference_angle_Z = 0
		},
		maxTargetDetectionRange = 450000,
		radar_type = 102,
		searchRadarFrequencies = { { 50000000, 54000000 }, { 2000000000, 2200000000 } },
		searchRadarMaxElevation = 1.0471975511966
	},
	Waypoint_Custom_Panel = true,
	Width = 43.9,
	X_nose = 109.2,
	X_tail = -88.5,
	_file = "./CoreMods/tech/SouthAtlanticAssets/Entries/Navy/ara_vdm.lua",
	_origin = "South_Atlantic_Assets",
	airFindDist = 18000,
	airWeaponDist = 5000,
	animation_arguments = {
		arresting_wires = { 400, 401, 402, 403, 414, 415 },
		blast_fences = {},
		catapult_shuttles = { 145 },
		flight_deck_fences = 88,
		holdback_bars_angle = { 808 },
		holdback_bars_color = { 812 },
		holdback_bars_deck_cleat = { 804 },
		holdback_bars_pos = { 817 },
		nav_lights = 69,
		radar1_rotation = 3,
		radar2_rotation = 2,
		radar3_rotation = 1,
		water_propeller = 8
	},
	armour_scheme = {
		hull_azimuth = { {} },
		hull_elevation = { {} },
		turret_azimuth = { {} },
		turret_elevation = { {} }
	},
	attribute = { 3, 12, 12, "Redacted", "Aircraft Carriers", "Armed Air Defence", "Arresting Gear", "catapult", "RADAR_BAND1_FOR_ARM", "RADAR_BAND2_FOR_ARM", "DetectionByAWACS", "AircraftCarrier", "AircraftCarrier With Catapult", "Armed Ship", "AircraftCarrier With Arresting Gear", "Naval", "All", "Ships", "Armed ships", "HeavyArmoredUnits", "Heavy armed ships" },
	carrierIlluminationStates = { { { 69, 0, 1 }, { 149, 0, 0.2 }, { 150, 0, 0 }, { 307, 0, 0 }, { 791, 0, 1 }, { 792, 0, 1 }, { 793, 0, 0 }, { 797, 0, 1 }, { 798, 0, 1 } }, { { 69, 0, 1 }, { 149, 0, 0.4 }, { 150, 0, 1 }, { 307, 0, 0.4 }, { 791, 0, 0 }, { 792, 0, 0 }, { 793, 0, 1 }, { 797, 0, 1 }, { 798, 0, 1 } }, { { 69, 0, 0 }, { 149, 0, 0.6 }, { 150, 0, 1 }, { 307, 0, 0.7 }, { 791, 0, 0 }, { 792, 0, 0 }, { 793, 0, 1 }, { 797, 0, 1 }, { 798, 0, 0 } } },
	chassis = {},
	distFindObstacles = 250,
	draft = 7.5,
	economy_distance = 13000,
	economy_velocity = 7.8,
	enablePlayerCanDrive = true,
	encyclopediaAnimation = {
		args = {
			[6] = 1,
			[41] = 0.5,
			[42] = 0.5,
			[43] = 0.5,
			[44] = 0.5,
			[45] = -0.5,
			[46] = -0.5,
			[47] = -0.5,
			[48] = -0.5,
			[49] = 1,
			[51] = 0,
			[52] = 0,
			[53] = 0,
			[54] = 0,
			[55] = 0,
			[56] = 0,
			[57] = 0,
			[58] = 0,
			[59] = 0
		}
	},
	exhaust = { {
			pos = { 23.04, 30.1, 11.88 },
			size = 0.4
		} },
	life = 7200,
	mapclasskey = "P0091000065",
	mass = 19000000,
	max_velocity = 12.4,
	numParking = 8,
	race_distance = 2778000,
	race_velocity = 9.25,
	radar1_period = 5,
	radar2_period = 7,
	radar3_period = 1,
	sensor = {
		max_alt_finding_target = 30000,
		max_range_finding_target = 50000,
		min_alt_finding_target = 0,
		min_range_finding_target = 0,
		pos = { -14.1, 16, 0 }
	},
	shipLength = 226.9,
	snd = {},
	speedup = 0.119249,
	swapped_names = true,
	tags = { "Carrier" },
	type = "ara_vdm",
	visual = {
		shape = "ara_vdm",
		shape_dstr = ""
	}
}