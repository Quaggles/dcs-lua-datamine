_G["db"]["Countries"]["#Index"] = {
	Awards = { <1>{
			countryID = 1,
			name = 'Medal "Zakhystnyku Vitchyzny"',
			nativeName = 'Medal "Zakhystnyku Vitchyzny"',
			picture = "Ukraine/awards/UKR-01-ForDefenderOfFatherland.png",
			threshold = 200
		}, <2>{
			countryID = 1,
			name = 'Orden "Za Zaslugy III"',
			nativeName = 'Orden "Za Zaslugy III"',
			picture = "Ukraine/awards/UKR-02-ForMerit-III.png",
			threshold = 600
		}, <3>{
			countryID = 1,
			name = 'Orden "Za Zaslugy II"',
			nativeName = 'Orden "Za Zaslugy II"',
			picture = "Ukraine/awards/UKR-03-ForMerit-II.png",
			threshold = 1000
		}, <4>{
			countryID = 1,
			name = 'Orden "Za Zaslugy I"',
			nativeName = 'Orden "Za Zaslugy I"',
			picture = "Ukraine/awards/UKR-04-ForMerit-I.png",
			threshold = 1400
		}, <5>{
			countryID = 1,
			name = 'Zirka "Za Zaslugy"',
			nativeName = 'Zirka "Za Zaslugy"',
			picture = "Ukraine/awards/UKR-05-ForMerit-Star.png",
			threshold = 1800
		}, <6>{
			countryID = 1,
			name = 'Medal "Za Viyskovu Sluzhbu"',
			nativeName = 'Medal "Za Viyskovu Sluzhbu"',
			picture = "Ukraine/awards/UKR-06-ForMilitaryService.png",
			threshold = 2200
		}, <7>{
			countryID = 1,
			name = 'Orden "Za Muzhnist"',
			nativeName = 'Orden "Za Muzhnist"',
			picture = "Ukraine/awards/UKR-07-OrderForCourage.png",
			threshold = 2600
		}, <8>{
			countryID = 1,
			name = "Zolota Zirka",
			nativeName = "Zolota Zirka",
			picture = "Ukraine/awards/UKR-08-GoldStar.png",
			threshold = 3000
		} },
	InternationalName = "Ukraine",
	Name = "Ukraine",
	OldID = "Ukraine",
	Ranks = { <9>{
			name = "Second lieutenant",
			nativeName = "Second lieutenant",
			pictureRect = { 0, 0, 64, 32 },
			stripes = "Ukraine/stripes.png",
			threshold = 0
		}, <10>{
			name = "First lieutenant",
			nativeName = "First lieutenant",
			pictureRect = { 0, 32, 64, 32 },
			stripes = "Ukraine/stripes.png",
			threshold = 15
		}, <11>{
			name = "Captain",
			nativeName = "Captain",
			pictureRect = { 0, 64, 64, 32 },
			stripes = "Ukraine/stripes.png",
			threshold = 30
		}, <12>{
			name = "Major",
			nativeName = "Major",
			pictureRect = { 0, 96, 64, 32 },
			stripes = "Ukraine/stripes.png",
			threshold = 60
		}, <13>{
			name = "Lieutenant colonel",
			nativeName = "Lieutenant colonel",
			pictureRect = { 0, 128, 64, 32 },
			stripes = "Ukraine/stripes.png",
			threshold = 120
		}, <14>{
			name = "Colonel",
			nativeName = "Colonel",
			pictureRect = { 0, 160, 64, 32 },
			stripes = "Ukraine/stripes.png",
			threshold = 240
		} },
	ShortName = "UKR",
	Troops = { <15>{
			name = "299 OShAP Saki",
			nativeName = "299 OShAP Saki",
			picture = "Ukraine/troops/299-OShAP-Saki.png"
		}, <16>{
			name = "Ukraine Air Force",
			nativeName = "Ukraine Air Force",
			picture = "Ukraine/troops/Ukraine-Airforce.png"
		}, <17>{
			name = "Ukrainian Falcons",
			nativeName = "Ukrainian Falcons",
			picture = "Ukraine/troops/Ukrainian-Falcons.png"
		}, <18>{
			name = "16 Army Aviation Brigade",
			nativeName = "16 Army Aviation Brigade",
			picture = "Ukraine/troops/16_OBRAA.png"
		}, <19>{
			name = "18 Army Aviation Brigade",
			nativeName = "18 Army Aviation Brigade",
			picture = "Ukraine/troops/18_OBRAA.png"
		}, <20>{
			name = "7 Army Aviation Regiment",
			nativeName = "7 Army Aviation Regiment",
			picture = "Ukraine/troops/7_OPAA.png"
		}, <21>{
			name = "11 Army Aviation Brigade",
			nativeName = "11 Army Aviation Brigade",
			picture = "Ukraine/troops/11_OBRAA.png"
		}, <22>{
			name = "Army Aviation",
			nativeName = "Army Aviation_ukr",
			picture = "Ukraine/troops/AA.png"
		}, <23>{
			name = "40 Tactical Aviation Brigade",
			nativeName = "40 Tactical Aviation Brigade",
			picture = "Ukraine/troops/40_BRTA.png"
		}, <24>{
			name = "114 Tactical Aviation Brigade",
			nativeName = "114 Tactical Aviation Brigade",
			picture = "Ukraine/troops/114_BRTA.png"
		}, <25>{
			name = "203 Aviation Training Brigade",
			nativeName = "203 Aviation Training Brigade",
			picture = "Ukraine/troops/203_NABR.png"
		}, <26>{
			name = "204 Tactical Aviation Brigade",
			nativeName = "204 Tactical Aviation Brigade",
			picture = "Ukraine/troops/204_BRTA.png"
		}, <27>{
			name = "831 Tactical Aviation Brigade",
			nativeName = "831 Tactical Aviation Brigade",
			picture = "Ukraine/troops/831_BRTA.png"
		} },
	Units = {
		ADEquipments = {
			ADEquipment = { {
					Name = "Ski Ramp"
				}, {
					Name = "SA Ski Ramp"
				} }
		},
		Animals = {
			Animal = { {
					Name = "Cow"
				} }
		},
		Cargos = {
			Cargo = { {
					Name = "uh1h_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ammo_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "barrels_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "m117_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "iso_container",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "iso_container_small",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "f_bar_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "container_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "tetrapod_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "fueltank_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "oiltank_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "pipes_small_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "pipes_big_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "trunks_small_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "trunks_long_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "cds_crate"
				}, {
					Name = "cds_barrels"
				}, {
					Name = "463_Pallet"
				}, {
					Name = "M92_Concrete_Barrier_Cargo"
				}, {
					Name = "M92_MRE_Pallet"
				}, {
					Name = "M92_Ammo_Pallet"
				}, {
					Name = "M92_10Ft_Container"
				}, {
					Name = "l118"
				} }
		},
		Cars = {
			Car = { {
					Name = "Bunker",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Sandbox",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "house1arm",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "house2arm",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "outpost_road",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "outpost_road_l",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "outpost_road_r",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "outpost",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "houseA_arm",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "TACAN_beacon",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "1L13 EWR",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "2B11 mortar",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "2S6 Tunguska",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "55G6 EWR",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ATMZ-5",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ATZ-10",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BMD-1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BMP-1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BMP-2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BMP-3",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BRDM-2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BTR_D",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BTR-80",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Dog Ear radar",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "GAZ-3307",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "GAZ-3308",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "GAZ-66",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Grad-URAL",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "IKARUS Bus",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "KAMAZ Truck",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "KrAZ6322",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Kub 1S91 str",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Kub 2P25 ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "LAZ Bus",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "LiAZ Bus",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MAZ-6303",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MTLB",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Osa 9A33 ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "p-19 s-125 sr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Paratrooper AKS-74",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Paratrooper RPG-16",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 40B6M tr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 40B6MD sr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 54K6 cp",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 5P85C ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 5P85D ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 64H6E sr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 5H63C 30H6_tr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 40B6MD sr_19J6",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SA-11 Buk CC 9S470M1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SA-11 Buk LN 9A310M1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SA-11 Buk SR 9S18M1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SA-18 Igla-S comm",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SA-18 Igla-S manpad",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SAU 2-C9",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SAU Akatsia",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SAU Gvozdika",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SAU Msta",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SKP-11",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Smerch",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Strela-1 9P31",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Strela-10M3",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "T-55",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "T-72B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "T-80UD",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tor 9A331",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Trolley bus",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "UAZ-469",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural ATsP-6",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-375 PBU",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-375 ZU-23",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-375",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-4320 APA-5D",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-4320T",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "VAZ Car",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZiL-131 APA-80",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZIL-131 KUNG",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZIL-4331",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZSU-23-4 Shilka",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZU-23 Emplacement Closed",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZU-23 Emplacement",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Smerch_HE",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hummer",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "M1043 HMMWV Armament",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "M1045 HMMWV TOW",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Uragan_BM-27",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "5p73 s-125 ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "snr s-125 tr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SA-18 Igla manpad",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SA-18 Igla comm",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-4320-31",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Scud_B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Electric locomotive",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Locomotive",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Coach a tank yellow",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Coach a tank blue",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Coach cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Coach cargo open",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Coach a platform",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Coach a passenger",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-200_Launcher",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "RPC_5N62V",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-60_Type59_Artillery",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "PT_76",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ATZ-5",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "generator_5i57",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZIL-135",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "RLS_19J6",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ATZ-60_Maz",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ATZ-60_TANK",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "TZ-22_KrAZ",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "TZ-22_TANK",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "tt_KORD",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "tt_DSHK",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "tt_B8M1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "tt_ZU-23",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HL_KORD",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HL_DSHK",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HL_B8M1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HL_ZU-23",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZSU_57_2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "M978 HEMTT Tanker",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MLRS",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Soldier stinger",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Stinger comm",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "TPZ",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "NASAMS_Command_Post",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "NASAMS_Radar_MPQ64F1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "NASAMS_LN_B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "NASAMS_LN_C",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "M-113",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hawk sr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hawk cwar",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hawk pcp",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hawk tr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hawk ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Gepard",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "M-109",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MaxxPro_MRAP",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "CHAP_FV107"
				}, {
					Name = "CHAP_T90M"
				}, {
					Name = "CHAP_M1083"
				}, {
					Name = "CHAP_MATV"
				}, {
					Name = "CHAP_TOS1A"
				}, {
					Name = "CHAP_PantsirS1"
				}, {
					Name = "CHAP_T84OplotM"
				}, {
					Name = "CHAP_T64BV"
				}, {
					Name = "T-90"
				}, {
					Name = "CHAP_IRISTSLM_STR"
				}, {
					Name = "CHAP_IRISTSLM_LN"
				}, {
					Name = "CHAP_IRISTSLM_CP"
				}, {
					Name = "CHAP_M142_GMLRS_M30"
				}, {
					Name = "CHAP_M142_GMLRS_M31"
				}, {
					Name = "CHAP_M142_ATACMS_M39A1"
				}, {
					Name = "CHAP_M142_ATACMS_M48"
				}, {
					Name = "TugHarlan_drivable"
				}, {
					Name = "B600_drivable"
				}, {
					Name = "MJ-1_drivable"
				}, {
					Name = "P20_drivable"
				}, {
					Name = "r11_volvo_drivable"
				}, {
					Name = "L118_Unit"
				}, {
					Name = "tacr2a"
				}, {
					Name = "LARC-V"
				}, {
					Name = "RD_75"
				}, {
					Name = "S_75M_Volhov"
				}, {
					Name = "SNR_75V"
				}, {
					Name = "ES44AH"
				}, {
					Name = "Boxcartrinity"
				}, {
					Name = "Tankcartrinity"
				}, {
					Name = "Wellcarnsc"
				}, {
					Name = "DR_50Ton_Flat_Wagon"
				}, {
					Name = "DRG_Class_86"
				}, {
					Name = "German_covered_wagon_G10"
				}, {
					Name = "German_tank_wagon"
				} }
		},
		Effects = {
			Effect = { {
					Name = "big_smoke",
					in_service = 0,
					out_of_service = 40000
				} }
		},
		Fortifications = {
			Fortification = { {
					Name = ".Command Center",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hangar A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tech hangar A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Farm A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Farm B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Garage A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Boiler-house A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Restaurant 1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Comms tower M",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Cafe",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Workshop A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Electric power box",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Supermarket A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Water tower A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "TV tower",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Shelter",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Repair workshop",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Railway station",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Railway crossing A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Railway crossing B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "WC",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 1A area",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 1A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 1B area",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 1B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 1C area",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 2C",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Shop",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tech combine",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Chemical tank A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small werehouse 1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small werehouse 2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small werehouse 3",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small werehouse 4",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Garage B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Garage small A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Garage small B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Pump station",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Oil derrick",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Container red 1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Container red 2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Container red 3",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Container white",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Container brown",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Barracks 2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Military staff",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hangar B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Fuel tank",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Shelter B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Oil platform",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure 1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure 2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure 3",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure C",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure D",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure E",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure F",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure G",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Landmine",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FARP Ammo Dump Coating",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FARP Tent",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FARP CP Blindage",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FARP Fuel Depot",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "GeneratorF",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Airshow_Cone",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Airshow_Crowd",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Red_Flag",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "White_Flag",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Black_Tyre",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "White_Tyre",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Black_Tyre_RF",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Black_Tyre_WF",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Windsock",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Beer Bomb"
				}, {
					Name = "Drop Zone Marker A"
				}, {
					Name = "Drop Zone Marker B"
				}, {
					Name = "Drop Zone Marker C"
				}, {
					Name = "Drop Zone Marker D"
				}, {
					Name = "Drop Zone Marker E"
				}, {
					Name = "AM32a-60_01"
				}, {
					Name = "AM32a-60_02"
				}, {
					Name = "APFC fuel"
				}, {
					Name = "B600"
				}, {
					Name = "Barrier A"
				}, {
					Name = "Barrier B"
				}, {
					Name = "Barrier C"
				}, {
					Name = "Barrier D"
				}, {
					Name = "BoomBarrier_open"
				}, {
					Name = "BoomBarrier_closed"
				}, {
					Name = "Building01_PBR"
				}, {
					Name = "Building02_PBR"
				}, {
					Name = "Building03_PBR"
				}, {
					Name = "Building04_PBR"
				}, {
					Name = "Building05_PBR"
				}, {
					Name = "Building06_PBR"
				}, {
					Name = "Building07_PBR"
				}, {
					Name = "Building08_PBR"
				}, {
					Name = "Cargo01"
				}, {
					Name = "Cargo02"
				}, {
					Name = "Cargo03"
				}, {
					Name = "Cargo04"
				}, {
					Name = "Cargo05"
				}, {
					Name = "Cargo06"
				}, {
					Name = "Camouflage01"
				}, {
					Name = "Camouflage02"
				}, {
					Name = "Camouflage03"
				}, {
					Name = "Camouflage04"
				}, {
					Name = "Camouflage05"
				}, {
					Name = "Camouflage06"
				}, {
					Name = "Camouflage07"
				}, {
					Name = "Cone01"
				}, {
					Name = "Cone02"
				}, {
					Name = "Container_10ft"
				}, {
					Name = "Container_20ft"
				}, {
					Name = "Container_40ft"
				}, {
					Name = "Container_watchtower"
				}, {
					Name = "Container_watchtower_lights"
				}, {
					Name = "Container_office"
				}, {
					Name = "Container_generator"
				}, {
					Name = "ElevatedPlatform_down"
				}, {
					Name = "ElevatedPlatform_up"
				}, {
					Name = "FireExtinguisher01"
				}, {
					Name = "FireExtinguisher02"
				}, {
					Name = "FireExtinguisher03"
				}, {
					Name = "HESCO_generator"
				}, {
					Name = "HESCO_post_1"
				}, {
					Name = "HESCO_wallperimeter_1"
				}, {
					Name = "HESCO_wallperimeter_2"
				}, {
					Name = "HESCO_wallperimeter_3"
				}, {
					Name = "HESCO_wallperimeter_4"
				}, {
					Name = "HESCO_wallperimeter_5"
				}, {
					Name = "HESCO_watchtower_1"
				}, {
					Name = "HESCO_watchtower_2"
				}, {
					Name = "HESCO_watchtower_3"
				}, {
					Name = "Jerrycan"
				}, {
					Name = "Ladder"
				}, {
					Name = "LHD_LHA"
				}, {
					Name = "M32-10C_01"
				}, {
					Name = "M32-10C_02"
				}, {
					Name = "M32-10C_03"
				}, {
					Name = "M32-10C_04"
				}, {
					Name = "MJ-1_01"
				}, {
					Name = "MJ-1_02"
				}, {
					Name = "NF-2_LightOn"
				}, {
					Name = "NF-2_LightOff01"
				}, {
					Name = "NF-2_LightOff02"
				}, {
					Name = "Oil Barrel"
				}, {
					Name = "Pile of Woods"
				}, {
					Name = "P20_01"
				}, {
					Name = "Revetment_x4"
				}, {
					Name = "Revetment_x8"
				}, {
					Name = "r11_volvo"
				}, {
					Name = "Sandbag_01"
				}, {
					Name = "Sandbag_02"
				}, {
					Name = "Sandbag_03"
				}, {
					Name = "Sandbag_04"
				}, {
					Name = "Sandbag_05"
				}, {
					Name = "Sandbag_06"
				}, {
					Name = "Sandbag_07"
				}, {
					Name = "Sandbag_08"
				}, {
					Name = "Sandbag_09"
				}, {
					Name = "Sandbag_10"
				}, {
					Name = "Sandbag_11"
				}, {
					Name = "Sandbag_12"
				}, {
					Name = "Sandbag_13"
				}, {
					Name = "Sandbag_15"
				}, {
					Name = "Sandbag_16"
				}, {
					Name = "Sandbag_17"
				}, {
					Name = "Shelter01"
				}, {
					Name = "Shelter02"
				}, {
					Name = "TugHarlan"
				}, {
					Name = "Tent05"
				}, {
					Name = "Tent04"
				}, {
					Name = "Tent03"
				}, {
					Name = "Tent02"
				}, {
					Name = "Tent01"
				}, {
					Name = "Toolbox01"
				}, {
					Name = "Toolbox02"
				}, {
					Name = "Twall_x1"
				}, {
					Name = "Twall_x6"
				}, {
					Name = "Twall_x6_3mts"
				}, {
					Name = "Orca"
				}, {
					Name = "WindTurbine"
				}, {
					Name = "WindTurbine_11"
				}, {
					Name = "billboard_motorized"
				}, {
					Name = "Stanley_LightHouse"
				}, {
					Name = "345 Excavator"
				}, {
					Name = "Tower Crane"
				}, {
					Name = "Small_LightHouse"
				}, {
					Name = "Nodding_Donkey_Pump"
				}, {
					Name = "offshore WindTurbine"
				}, {
					Name = "offshore WindTurbine2"
				}, {
					Name = "FarpHide_small"
				}, {
					Name = "FarpHide_Med"
				}, {
					Name = "FarpHide_Dsmall"
				}, {
					Name = "FarpHide_Dmed"
				}, {
					Name = "container_40ft"
				}, {
					Name = "container_20ft"
				}, {
					Name = "FlagPole"
				}, {
					Name = "ip_tower"
				}, {
					Name = "af_hq"
				}, {
					Name = "Concertina wire"
				}, {
					Name = "Haystack 1"
				}, {
					Name = "Haystack 2"
				}, {
					Name = "Haystack 3"
				}, {
					Name = "Haystack 4"
				} }
		},
		GrassAirfields = {
			GrassAirfield = { {
					Name = "GrassAirfield",
					in_service = 0,
					out_of_service = 40000
				} }
		},
		Helicopters = {
			Helicopter = { {
					Name = "Ka-50",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Mi-24V",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Mi-8MT",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Mi-26",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ka-27",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "UH-1H",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Mi-24P",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "AH-64D_BLK_II"
				}, {
					Name = "CH-47Fbl1"
				}, {
					Name = "Ka-50_3"
				}, {
					Name = "OH58D"
				}, {
					Name = "SA342M"
				}, {
					Name = "SA342L"
				}, {
					Name = "SA342Mistral"
				}, {
					Name = "SA342Minigun"
				} }
		},
		Heliports = {
			Heliport = { {
					Name = "FARP",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SINGLE_HELIPAD",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FARP_SINGLE_01"
				}, {
					Name = "Oil rig"
				}, {
					Name = "Gas platform"
				}, {
					Name = "Invisible FARP"
				} }
		},
		LTAvehicles = {
			LTAvehicle = { {
					Name = "m1_vla"
				} }
		},
		Personnel = {
			Personnel = {}
		},
		Planes = {
			Plane = { {
					Name = "A-10C",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Su-27",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-29A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-29S",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Su-17M4",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tu-95MS",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Su-24M",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Su-24MR",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Su-25",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-25PD",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "An-26B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "An-30M",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-23MLD",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "IL-78M",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "IL-76MD",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-27K",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tu-22M3",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-25RBT",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Yak-40",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "L-39ZA",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "P-51D",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "L-39C",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tu-142",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tu-160",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Yak-52",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FW-190A8"
				}, {
					Name = "Bf-109K-4"
				}, {
					Name = "SpitfireLFMkIX"
				}, {
					Name = "SpitfireLFMkIXCW"
				}, {
					Name = "P-51D-30-NA"
				}, {
					Name = "P-47D-30"
				}, {
					Name = "P-47D-30bl1"
				}, {
					Name = "P-47D-40"
				}, {
					Name = "MosquitoFBMkVI"
				}, {
					Name = "A-20G"
				}, {
					Name = "A-10A"
				}, {
					Name = "A-10C_2"
				}, {
					Name = "AJS37"
				}, {
					Name = "AV8BNA"
				}, {
					Name = "KC130"
				}, {
					Name = "KC135MPRS"
				}, {
					Name = "C-101EB"
				}, {
					Name = "C-101CC"
				}, {
					Name = "JF-17"
				}, {
					Name = "Falcon_Gyrocopter"
				}, {
					Name = "Christen Eagle II"
				}, {
					Name = "F-15ESE"
				}, {
					Name = "F-16C_50"
				}, {
					Name = "F-4E-45MC"
				}, {
					Name = "QF-4E"
				}, {
					Name = "F-5E"
				}, {
					Name = "F-5E-3"
				}, {
					Name = "F-5E-3_FC"
				}, {
					Name = "F-86F Sabre"
				}, {
					Name = "F-86F_FC"
				}, {
					Name = "F4U-1D"
				}, {
					Name = "F4U-1D_CW"
				}, {
					Name = "F/A-18A"
				}, {
					Name = "F/A-18C"
				}, {
					Name = "FA-18C_hornet"
				}, {
					Name = "Hawk"
				}, {
					Name = "I-16"
				}, {
					Name = "M-2000C"
				}, {
					Name = "MB-339A"
				}, {
					Name = "MB-339APAN"
				}, {
					Name = "MiG-15bis"
				}, {
					Name = "MiG-15bis_FC"
				}, {
					Name = "MiG-19P"
				}, {
					Name = "MiG-21Bis"
				}, {
					Name = "MiG-29 Fulcrum"
				}, {
					Name = "Mirage-F1CE"
				}, {
					Name = "Mirage-F1EE"
				}, {
					Name = "Mirage-F1BE"
				}, {
					Name = "TF-51D"
				}, {
					Name = "Ju-88A4"
				}, {
					Name = "C-47"
				} }
		},
		Ships = {
			Ship = { {
					Name = "speedboat",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ELNYA",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ALBATROS",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MOLNIYA",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "KILO",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZWEZDNY",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Dry-cargo ship-1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Dry-cargo ship-2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "REZKY",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BDK-775",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HarborTug"
				}, {
					Name = "Ship_Tilde_Supply"
				} }
		},
		WWIIstructures = {
			WWIIstructure = {}
		},
		Warehouses = {
			Warehouse = { {
					Name = "Warehouse",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tank",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = ".Ammunition depot",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tank 2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tank 3",
					in_service = 0,
					out_of_service = 40000
				} }
		}
	},
	WorldID = 1,
	award_by_name = {
		['Medal "Za Viyskovu Sluzhbu"'] = <table 6>,
		['Medal "Zakhystnyku Vitchyzny"'] = <table 1>,
		['Orden "Za Muzhnist"'] = <table 7>,
		['Orden "Za Zaslugy I"'] = <table 4>,
		['Orden "Za Zaslugy II"'] = <table 3>,
		['Orden "Za Zaslugy III"'] = <table 2>,
		['Zirka "Za Zaslugy"'] = <table 5>,
		["Zolota Zirka"] = <table 8>
	},
	flag = "FUI/Common/Flags/Ukraine.png",
	flag_small = "MissionEditor/data/images/flags/Ukraine.png",
	rank_by_name = {
		Captain = <table 11>,
		Colonel = <table 14>,
		["First lieutenant"] = <table 10>,
		["Lieutenant colonel"] = <table 13>,
		Major = <table 12>,
		["Second lieutenant"] = <table 9>
	},
	troop_by_name = {
		["11 Army Aviation Brigade"] = <table 21>,
		["114 Tactical Aviation Brigade"] = <table 24>,
		["16 Army Aviation Brigade"] = <table 18>,
		["18 Army Aviation Brigade"] = <table 19>,
		["203 Aviation Training Brigade"] = <table 25>,
		["204 Tactical Aviation Brigade"] = <table 26>,
		["299 OShAP Saki"] = <table 15>,
		["40 Tactical Aviation Brigade"] = <table 23>,
		["7 Army Aviation Regiment"] = <table 20>,
		["831 Tactical Aviation Brigade"] = <table 27>,
		["Army Aviation"] = <table 22>,
		["Ukraine Air Force"] = <table 16>,
		["Ukrainian Falcons"] = <table 17>
	}
}