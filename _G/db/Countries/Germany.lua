_G["db"]["Countries"]["#Index"] = {
	Awards = { <1>{
			countryID = 6,
			name = "Ehrenmedaille",
			nativeName = "Ehrenmedaille",
			picture = "Germany/awards/DE-01-Ehrenmedaille-Honor.png",
			threshold = 200
		}, <2>{
			countryID = 6,
			name = "Ehrenkreuz",
			nativeName = "Ehrenkreuz",
			picture = "Germany/awards/DE-02-Ehrenkreuze.png",
			threshold = 700
		}, <3>{
			countryID = 6,
			name = "Ehrenkreuz in Silber",
			nativeName = "Ehrenkreuz in Silber",
			picture = "Germany/awards/DE-03-Silbernekreuze.png",
			threshold = 1200
		}, <4>{
			countryID = 6,
			name = "Ehrenkreuz in Gold",
			nativeName = "Ehrenkreuz in Gold",
			picture = "Germany/awards/DE-04-Goldenekreuze.png",
			threshold = 1800
		}, <5>{
			countryID = 6,
			name = "Bundesverdienstmedaille",
			nativeName = "Bundesverdienstmedaille",
			picture = "Germany/awards/DE-05-Bundesverdienstmedaille.png",
			threshold = 2400
		}, <6>{
			countryID = 6,
			name = "Bundesverdienstkreuz",
			nativeName = "Bundesverdienstkreuz",
			picture = "Germany/awards/DE-06-Bundesverdienstkreuz.png",
			threshold = 3000
		} },
	InternationalName = "Germany",
	Name = "Germany",
	OldID = "Germany",
	Ranks = { <7>{
			name = "Second lieutenant",
			nativeName = "Second lieutenant",
			pictureRect = { 0, 0, 64, 32 },
			stripes = "Germany/stripes.png",
			threshold = 0
		}, <8>{
			name = "First lieutenant",
			nativeName = "First lieutenant",
			pictureRect = { 0, 32, 64, 32 },
			stripes = "Germany/stripes.png",
			threshold = 15
		}, <9>{
			name = "Captain",
			nativeName = "Captain",
			pictureRect = { 0, 64, 64, 32 },
			stripes = "Germany/stripes.png",
			threshold = 30
		}, <10>{
			name = "Major",
			nativeName = "Major",
			pictureRect = { 0, 96, 64, 32 },
			stripes = "Germany/stripes.png",
			threshold = 60
		}, <11>{
			name = "Lieutenant colonel",
			nativeName = "Lieutenant colonel",
			pictureRect = { 0, 128, 64, 32 },
			stripes = "Germany/stripes.png",
			threshold = 120
		}, <12>{
			name = "Colonel",
			nativeName = "Colonel",
			pictureRect = { 0, 160, 64, 32 },
			stripes = "Germany/stripes.png",
			threshold = 240
		} },
	ShortName = "GER",
	Troops = { <13>{
			name = "Section Break",
			nativeName = "*Bundeswehr Luftwaffe*",
			picture = "Germany/troops/Bundeswehr_Luftwaffe.png"
		}, <14>{
			name = "FlBschBMVg",
			nativeName = "FlBschBMVg",
			picture = "Germany/troops/FlBschBMVg.png"
		}, <15>{
			name = "FlgAusbZLw",
			nativeName = "FlgAusbZLw",
			picture = "Germany/troops/FlgAusbZLw.png"
		}, <16>{
			name = "HSG 64",
			nativeName = "HSG 64",
			picture = "Germany/troops/HSG_64.png"
		}, <17>{
			name = "LTG 61",
			nativeName = "LTG 61",
			picture = "Germany/troops/LTG_61.png"
		}, <18>{
			name = "LTG 62",
			nativeName = "LTG 62",
			picture = "Germany/troops/LTG_62.png"
		}, <19>{
			name = "LTG 63",
			nativeName = "LTG 63",
			picture = "Germany/troops/LTG_63.png"
		}, <20>{
			name = "TaktLwG 31",
			nativeName = "TaktLwG 31",
			picture = "Germany/troops/TaktLwG_31.png"
		}, <21>{
			name = "TaktLwG 33",
			nativeName = "TaktLwG 33",
			picture = "Germany/troops/TaktLwG_33.png"
		}, <22>{
			name = "TaktLwG 51",
			nativeName = "TaktLwG 51",
			picture = "Germany/troops/TaktLwG_51.png"
		}, <23>{
			name = "TaktLwG 71",
			nativeName = "TaktLwG 71",
			picture = "Germany/troops/TaktLwG_71.png"
		}, <24>{
			name = "TaktLwG 73",
			nativeName = "TaktLwG 73",
			picture = "Germany/troops/TaktLwG_73.png"
		}, <25>{
			name = "TaktLwG 74",
			nativeName = "TaktLwG 74",
			picture = "Germany/troops/TaktLwG_74.png"
		} },
	Units = {
		ADEquipments = {
			ADEquipment = {}
		},
		Animals = {
			Animal = { {
					Name = "Cow"
				} }
		},
		Cargos = {
			Cargo = { {
					Name = "uh1h_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ammo_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "barrels_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "m117_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "iso_container",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "iso_container_small",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "f_bar_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "container_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "tetrapod_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "fueltank_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "oiltank_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "pipes_small_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "pipes_big_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "trunks_small_cargo",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "trunks_long_cargo",
					in_service = 0,
					out_of_service = 40000
				} }
		},
		Cars = {
			Car = { {
					Name = "Bunker",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Sandbox",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "house1arm",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "house2arm",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "outpost_road",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "outpost",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "houseA_arm",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "TACAN_beacon",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "M-113",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Patriot str",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Patriot ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Patriot AMG",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Patriot EPP",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Patriot ECS",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Patriot cp",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hawk sr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hawk cwar",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hawk pcp",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hawk tr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hawk ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Roland ADS",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Roland Radar",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Gepard",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hummer",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Leopard-2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Leopard1A3",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Leopard-2A5",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "M-109",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Marder",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "TPZ",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MLRS",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MLRS FDDM",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Soldier stinger",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Stinger comm",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "M 818",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "M978 HEMTT Tanker",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HEMTT TFFT",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "p-19 s-125 sr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MTLB",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BRDM-2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BMP-1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "BMP-2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "T-55",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "2B11 mortar",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Grad-URAL",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SAU Gvozdika",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SAU Akatsia",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "bofors40",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZU-23 Emplacement Closed",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZU-23 Emplacement",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-375 ZU-23",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SNR_75V",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S_75M_Volhov",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "5p73 s-125 ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "snr s-125 tr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Kub 2P25 ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Kub 1S91 str",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Osa 9A33 ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Strela-1 9P31",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 5P85C ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 5P85D ln",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 54K6 cp",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 40B6M tr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 40B6MD sr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "S-300PS 64H6E sr",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "rapier_fsa_launcher",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "rapier_fsa_optical_tracker_unit",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "rapier_fsa_blindfire_radar",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZSU-23-4 Shilka",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-4320 APA-5D",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZiL-131 APA-80",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ATMZ-5",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ATZ-10",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Blitz_36-6700A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "GAZ-66",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Horch_901_typ_40_kfz_21",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "KAMAZ Truck",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "UAZ-469",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural ATsP-6",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-375",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-4320-31",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Ural-4320T",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ZIL-131 KUNG",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Scud_B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "soldier_mauser98",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "DRG_Class_86",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "German_covered_wagon_G10",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "DR_50Ton_Flat_Wagon",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "German_tank_wagon",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "ATZ-5",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "leopard-2A4",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "tt_KORD",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "tt_DSHK",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "tt_B8M1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "tt_ZU-23",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HL_KORD",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HL_DSHK",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HL_B8M1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HL_ZU-23",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Electric locomotive"
				}, {
					Name = "Locomotive"
				}, {
					Name = "Coach cargo"
				}, {
					Name = "Coach cargo open"
				}, {
					Name = "Coach a tank blue"
				}, {
					Name = "Coach a tank yellow"
				}, {
					Name = "Coach a passenger"
				}, {
					Name = "Coach a platform"
				}, {
					Name = "RPC_5N62V"
				}, {
					Name = "S-200_Launcher"
				}, {
					Name = "ES44AH"
				}, {
					Name = "Boxcartrinity"
				}, {
					Name = "Tankcartrinity"
				}, {
					Name = "Wellcarnsc"
				}, {
					Name = "flak18"
				}, {
					Name = "Pz_IV_H"
				}, {
					Name = "Sd_Kfz_251"
				}, {
					Name = "Kubelwagen_82"
				}, {
					Name = "Sd_Kfz_2"
				}, {
					Name = "Sd_Kfz_7"
				}, {
					Name = "Tiger_I"
				}, {
					Name = "Tiger_II_H"
				}, {
					Name = "Pz_V_Panther_G"
				}, {
					Name = "Jagdpanther_G1"
				}, {
					Name = "JagdPz_IV"
				}, {
					Name = "Stug_IV"
				}, {
					Name = "SturmPzIV"
				}, {
					Name = "Wespe124"
				}, {
					Name = "Sd_Kfz_234_2_Puma"
				}, {
					Name = "flak30"
				}, {
					Name = "flak36"
				}, {
					Name = "flak37"
				}, {
					Name = "flak38"
				}, {
					Name = "KDO_Mod40"
				}, {
					Name = "Flakscheinwerfer_37"
				}, {
					Name = "Maschinensatz_33"
				}, {
					Name = "SK_C_28_naval_gun"
				}, {
					Name = "fire_control"
				}, {
					Name = "Stug_III"
				}, {
					Name = "Elefant_SdKfz_184"
				}, {
					Name = "flak41"
				}, {
					Name = "v1_launcher"
				}, {
					Name = "FuMG-401"
				}, {
					Name = "FuSe-65"
				}, {
					Name = "Pak40"
				}, {
					Name = "LeFH_18-40-105"
				} }
		},
		Effects = {
			Effect = { {
					Name = "big_smoke",
					in_service = 0,
					out_of_service = 40000
				} }
		},
		Fortifications = {
			Fortification = { {
					Name = ".Command Center",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hangar A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tech hangar A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Farm A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Farm B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Garage A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Boiler-house A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Restaurant 1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Comms tower M",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Cafe",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Workshop A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Electric power box",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Supermarket A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Water tower A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "TV tower",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Shelter",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Repair workshop",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Railway station",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Railway crossing A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Railway crossing B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "WC",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 1A area",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 1A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 1B area",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 1B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 1C area",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small house 2C",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Shop",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tech combine",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Chemical tank A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small werehouse 1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small werehouse 2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small werehouse 3",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Small werehouse 4",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Garage B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Garage small A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Garage small B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Pump station",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Oil derrick",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Container red 1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Container red 2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Container red 3",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Container white",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Container brown",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Barracks 2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Military staff",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Hangar B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Fuel tank",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Shelter B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Oil platform",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure 1",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure 2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure 3",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure C",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure D",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure E",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure F",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Subsidiary structure G",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Landmine",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FARP Ammo Dump Coating",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FARP Tent",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FARP CP Blindage",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FARP Fuel Depot",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "GeneratorF",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Airshow_Cone",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Airshow_Crowd",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Red_Flag",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "White_Flag",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Black_Tyre",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "White_Tyre",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Black_Tyre_RF",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Black_Tyre_WF",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Windsock",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Beer Bomb"
				}, {
					Name = "Belgian gate"
				}, {
					Name = "Concertina wire"
				}, {
					Name = "Tetrahydra"
				}, {
					Name = "Hemmkurvenhindernis"
				}, {
					Name = "Czech hedgehogs 1"
				}, {
					Name = "Czech hedgehogs 2"
				}, {
					Name = "Dragonteeth 1"
				}, {
					Name = "Dragonteeth 2"
				}, {
					Name = "Dragonteeth 3"
				}, {
					Name = "Dragonteeth 4"
				}, {
					Name = "Dragonteeth 5"
				}, {
					Name = "Fire Control Bunker"
				}, {
					Name = "Haystack 1"
				}, {
					Name = "Haystack 2"
				}, {
					Name = "Haystack 3"
				}, {
					Name = "Haystack 4"
				}, {
					Name = "Log posts 1"
				}, {
					Name = "Log posts 2"
				}, {
					Name = "Log posts 3"
				}, {
					Name = "Log ramps 1"
				}, {
					Name = "Log ramps 2"
				}, {
					Name = "Log ramps 3"
				}, {
					Name = "Siegfried Line"
				}, {
					Name = "Freya_Shelter_Brick"
				}, {
					Name = "Freya_Shelter_Concrete"
				} }
		},
		GrassAirfields = {
			GrassAirfield = { {
					Name = "GrassAirfield",
					in_service = 0,
					out_of_service = 40000
				} }
		},
		Helicopters = {
			Helicopter = { {
					Name = "Ka-50",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Mi-8MT",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "UH-1H",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Mi-24P",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "AH-64D_BLK_II"
				}, {
					Name = "SA342M"
				}, {
					Name = "SA342L"
				}, {
					Name = "SA342Mistral"
				}, {
					Name = "SA342Minigun"
				} }
		},
		Heliports = {
			Heliport = { {
					Name = "FARP",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "SINGLE_HELIPAD",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Oil rig"
				}, {
					Name = "Gas platform"
				}, {
					Name = "Invisible FARP"
				} }
		},
		LTAvehicles = {
			LTAvehicle = { {
					Name = "m1_vla"
				} }
		},
		Personnel = {
			Personnel = {}
		},
		Planes = {
			Plane = { {
					Name = "A-10C",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-29G",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "F-4E",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tornado IDS",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "P-51D",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "An-26B",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "C-17A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "E-3A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-15bis",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-15bis_MAC",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-21Bis",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Su-17M4",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Yak-40",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Yak-52",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "F-4E_new",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "MiG-29A",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "C-47",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "FW-190D9"
				}, {
					Name = "FW-190A8"
				}, {
					Name = "Bf-109K-4"
				}, {
					Name = "SpitfireLFMkIX"
				}, {
					Name = "SpitfireLFMkIXCW"
				}, {
					Name = "P-51D-30-NA"
				}, {
					Name = "P-47D-30"
				}, {
					Name = "P-47D-30bl1"
				}, {
					Name = "P-47D-40"
				}, {
					Name = "MosquitoFBMkVI"
				}, {
					Name = "A-20G"
				}, {
					Name = "A-10A"
				}, {
					Name = "A-10C_2"
				}, {
					Name = "AJS37"
				}, {
					Name = "AV8BNA"
				}, {
					Name = "KC130"
				}, {
					Name = "KC135MPRS"
				}, {
					Name = "C-101EB"
				}, {
					Name = "C-101CC"
				}, {
					Name = "JF-17"
				}, {
					Name = "Christen Eagle II"
				}, {
					Name = "F-16C_50"
				}, {
					Name = "F-5E"
				}, {
					Name = "F-5E-3"
				}, {
					Name = "F-86F Sabre"
				}, {
					Name = "FA-18C_hornet"
				}, {
					Name = "Hawk"
				}, {
					Name = "I-16"
				}, {
					Name = "L-39C"
				}, {
					Name = "M-2000C"
				}, {
					Name = "MiG-19P"
				}, {
					Name = "Ju-88A4"
				}, {
					Name = "TF-51D"
				} }
		},
		Ships = {
			Ship = { {
					Name = "speedboat",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "HandyWind",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Seawise_Giant",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "La_Combattante_II",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Uboat_VIIC"
				}, {
					Name = "Schnellboot_type_S130"
				} }
		},
		WWIIstructures = {
			WWIIstructure = {}
		},
		Warehouses = {
			Warehouse = { {
					Name = "Warehouse",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tank",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = ".Ammunition depot",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tank 2",
					in_service = 0,
					out_of_service = 40000
				}, {
					Name = "Tank 3",
					in_service = 0,
					out_of_service = 40000
				} }
		}
	},
	WorldID = 6,
	award_by_name = {
		Bundesverdienstkreuz = <table 6>,
		Bundesverdienstmedaille = <table 5>,
		Ehrenkreuz = <table 2>,
		["Ehrenkreuz in Gold"] = <table 4>,
		["Ehrenkreuz in Silber"] = <table 3>,
		Ehrenmedaille = <table 1>
	},
	flag = "FUI/Common/Flags/Germany.png",
	flag_small = "MissionEditor/data/images/flags/Germany.png",
	rank_by_name = {
		Captain = <table 9>,
		Colonel = <table 12>,
		["First lieutenant"] = <table 8>,
		["Lieutenant colonel"] = <table 11>,
		Major = <table 10>,
		["Second lieutenant"] = <table 7>
	},
	troop_by_name = {
		FlBschBMVg = <table 14>,
		FlgAusbZLw = <table 15>,
		["HSG 64"] = <table 16>,
		["LTG 61"] = <table 17>,
		["LTG 62"] = <table 18>,
		["LTG 63"] = <table 19>,
		["Section Break"] = <table 13>,
		["TaktLwG 31"] = <table 20>,
		["TaktLwG 33"] = <table 21>,
		["TaktLwG 51"] = <table 22>,
		["TaktLwG 71"] = <table 23>,
		["TaktLwG 73"] = <table 24>,
		["TaktLwG 74"] = <table 25>
	}
}