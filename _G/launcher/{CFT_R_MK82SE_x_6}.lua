_G["launcher"]["{CFT_R_MK82SE_x_6}"] = {
	CLSID = "{CFT_R_MK82SE_x_6}",
	Count = 6,
	Cx_pil = 0.001116,
	DistributeOverCarrierDirectly = true,
	Elements = { {
			DrawArgs = { { 1, 1 }, { 2, 1 } },
			ShapeName = "MK-82_Snakeye",
			connector_name = "Pylon15"
		}, {
			DrawArgs = { { 1, 1 }, { 2, 1 } },
			ShapeName = "MK-82_Snakeye",
			connector_name = "Pylon12"
		}, {
			DrawArgs = { { 1, 1 }, { 2, 1 } },
			ShapeName = "MK-82_Snakeye",
			connector_name = "Pylon16"
		}, {
			DrawArgs = { { 1, 1 }, { 2, 1 } },
			ShapeName = "MK-82_Snakeye",
			connector_name = "Pylon14"
		}, {
			DrawArgs = { { 1, 1 }, { 2, 1 } },
			ShapeName = "MK-82_Snakeye",
			connector_name = "Pylon13"
		}, {
			DrawArgs = { { 1, 1 }, { 2, 1 } },
			ShapeName = "MK-82_Snakeye",
			connector_name = "Pylon11"
		} },
	Picture = "mk82AIR.png",
	Weight = 1446,
	Weight_Empty = 0,
	_file = "./CoreMods/aircraft/F-15E/F-15E.lua",
	_origin = "F-15E AI by RAZBAM",
	attribute = <1>{ 4, 5, 9, "Redacted" },
	category = 1,
	displayName = "Mk-82 SnakeEye * 6",
	wsTypeOfWeapon = <table 1>
}