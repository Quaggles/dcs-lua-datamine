_G["launcher"]["{BRU42LS_2*LAU68_HYDRA_70_M151_L}"] = {
	CLSID = "{BRU42LS_2*LAU68_HYDRA_70_M151_L}",
	Count = 2,
	Cx_pil = 0.0034296875,
	Elements = { {
			IsAdapter = true,
			ShapeName = "BRU-42_LS"
		}, {
			connector_name = "Point03",
			payload_CLSID = "{A021F29D-18AB-4d3e-985C-FC9C60E35E9E}"
		}, {
			connector_name = "Point01",
			payload_CLSID = "{A021F29D-18AB-4d3e-985C-FC9C60E35E9E}"
		} },
	Picture = "LAU68.png",
	Weight = 284.05,
	Weight_Empty = 137.05,
	_file = "./CoreMods/aircraft/AircraftWeaponPack/rockets.lua",
	_origin = "AircraftWeaponPack",
	attribute = { 4, 7, 32, "Redacted" },
	category = 3,
	displayName = 'BRU-42 with 2 x LAU-68 pod - 7 x 2.75" Hydra, UnGd Rkts M151, HE',
	name = "{BRU42LS_2*LAU68_HYDRA_70_M151_L}",
	wsTypeOfWeapon = { 4, 7, 33, "Redacted" }
}