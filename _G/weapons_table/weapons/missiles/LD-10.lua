_G["weapons_table"]["weapons"]["missiles"]["LD-10"] = {
	Reflection = 0.0329,
	_file = "./CoreMods/aircraft/ChinaAssetPack/Entries/Loadouts/mils_ld10.lua",
	_origin = "China Asset Pack by Deka Ironwork Simulations and Eagle Dynamics",
	_unique_resource_name = "weapons.missiles.LD-10",
	caliber = 0.203,
	client = {
		Cx_pil = 2,
		D_max = 80000,
		D_min = 700,
		Diam = 203,
		Escort = 0,
		Fi_excort = 1.05,
		Fi_rak = 3.14152,
		Fi_search = 1.05,
		Fi_start = 0.5236,
		H_max = 25000,
		H_min = -1,
		H_min_t = 1,
		Head_Form = 1,
		Head_Type = 3,
		KillDistance = 7,
		LaunchDistData = { 9, 6, 100, 150, 200, 250, 300, 350, 1000, 12000, 13000, 16000, 16000, 17000, 30000, 2000, 17000, 19000, 25000, 30000, 35000, 40000, 4000, 20000, 22000, 25000, 30000, 35000, 45000, 6000, 30000, 32000, 35000, 37000, 40000, 50000, 7000, 40000, 42000, 45000, 47000, 55000, 60000, 8000, 45000, 47000, 50000, 55000, 60000, 65000, 9000, 50000, 52500, 55000, 58000, 65000, 70000, 10000, 60000, 62500, 65000, 68000, 70000, 75000, 12000, 65000, 66000, 67000, 70000, 75000, 80000 },
		Life_Time = 180,
		M = 199,
		Mach_max = 4,
		Nr_max = 25,
		OmViz_max = 0.52,
		Range_max = 80000,
		Reflection = 0.0329,
		X_back = -1.74,
		Y_back = -0.11,
		Z_back = 0,
		_file = "./CoreMods/aircraft/ChinaAssetPack/Entries/Loadouts/mils_ld10.lua",
		_origin = "China Asset Pack by Deka Ironwork Simulations and Eagle Dynamics",
		autopilot = {
			J_Angle_K = 0.20943951023932,
			J_Angle_W = 3.5,
			J_Diff_K = 0.4,
			J_FinAngle_K = 0.31415926535898,
			J_Int_K = 0,
			J_Power_K = 1.2,
			K = 100,
			Kg = 6,
			Ki = 0,
			delay = 1,
			finsLimit = 0.22,
			useJumpByDefault = 1
		},
		category = 2,
		class_name = "wAmmunitionSelfHoming",
		controller = {
			march_start = 0.021
		},
		exhaust = { 1, 1, 1, 0.5 },
		fm = {
			A = 0.36,
			I = 239.46333333333,
			Kw_x = 0.001,
			L = 3.8,
			Ma = 0.3,
			Ma_x = 0.001,
			Mw = 1.116,
			Sw = 0.85,
			caliber = 0.203,
			cx_coeff = { 1, 0.4, 1.1, 0.5, 1.4 },
			dCydA = { 0.07, 0.036 },
			finsTau = 0.1,
			mass = 199,
			maxAoa = 0.28,
			wind_sigma = 0,
			wind_time = 0
		},
		fuze_proximity = {
			arm_delay = 10,
			ignore_inp_armed = 1,
			radius = 10
		},
		march = {
			boost_factor = 0,
			boost_time = 0,
			custom_smoke_dissipation_factor = 0.2,
			fuel_mass = 70,
			impulse = 205,
			nozzle_orientationXYZ = { { 0, 0, 0 } },
			nozzle_position = { { -1.74, -0.11, 0 } },
			smoke_color = { 0.6, 0.6, 0.6 },
			smoke_transparency = 0.8,
			tail_width = 0.3,
			work_time = 10
		},
		mass = 199,
		model = "ld10",
		name = "LD-10",
		radio_seeker = {
			FOV = 0.087266462599716,
			aim_y_offset = 2,
			blind_ctrl_dist = 1000,
			blind_rad_val = 0.2,
			calc_aim_dist = 500000,
			err_correct_time = 1,
			err_val = 0.001,
			keep_aim_time = 8,
			min_sens_rad_val = 0.00025,
			op_time = 150,
			pos_memory_time = 60,
			sens_far_dist = 60000,
			sens_near_dist = 200
		},
		scheme = "schemes/missiles/anti_radiation_missile2.sch",
		shape_table_data = { {
				file = "ld10",
				fire = { 0, 1 },
				index = "Redacted",
				life = 1,
				name = "LD-10",
				username = "LD-10"
			} },
		sigma = { 10, 10, 10 },
		simple_gyrostab_seeker = {
			omega_max = 0.13962634015955
		},
		start_helper = {
			delay = 0.2,
			max_height = 400,
			max_vel = 200,
			power = 0.02,
			time = 2,
			use_local_coord = 0,
			vh_logic_or = 1
		},
		t_acc = 3,
		t_b = 0,
		t_marsh = 5,
		user_name = "LD-10",
		v_mid = 900,
		v_min = 140,
		warhead = {
			caliber = 90,
			concrete_factors = { 1, 1, 1 },
			concrete_obj_factor = 1,
			cumulative_factor = 2,
			cumulative_thickness = 0,
			expl_mass = 20,
			fantom = 1,
			mass = 20,
			obj_factors = { 2, 1 },
			other_factors = { 1, 1, 1 },
			piercing_mass = 0
		},
		warhead_air = {
			caliber = 90,
			concrete_factors = { 1, 1, 1 },
			concrete_obj_factor = 1,
			cumulative_factor = 2,
			cumulative_thickness = 0,
			expl_mass = 20,
			fantom = 1,
			mass = 20,
			obj_factors = { 2, 1 },
			other_factors = { 1, 1, 1 },
			piercing_mass = 0
		},
		wsTypeOfWeapon = { 4, 4, 8, "Redacted" }
	},
	display_name = "LD-10",
	mass = 199,
	model = "ld10",
	name = "LD-10",
	server = {
		Cx_pil = 2,
		D_max = 80000,
		D_min = 700,
		Diam = 203,
		Escort = 0,
		Fi_excort = 1.05,
		Fi_rak = 3.14152,
		Fi_search = 1.05,
		Fi_start = 0.5236,
		H_max = 25000,
		H_min = -1,
		H_min_t = 1,
		Head_Form = 1,
		Head_Type = 3,
		KillDistance = 7,
		LaunchDistData = { 9, 6, 100, 150, 200, 250, 300, 350, 1000, 12000, 13000, 16000, 16000, 17000, 30000, 2000, 17000, 19000, 25000, 30000, 35000, 40000, 4000, 20000, 22000, 25000, 30000, 35000, 45000, 6000, 30000, 32000, 35000, 37000, 40000, 50000, 7000, 40000, 42000, 45000, 47000, 55000, 60000, 8000, 45000, 47000, 50000, 55000, 60000, 65000, 9000, 50000, 52500, 55000, 58000, 65000, 70000, 10000, 60000, 62500, 65000, 68000, 70000, 75000, 12000, 65000, 66000, 67000, 70000, 75000, 80000 },
		Life_Time = 180,
		M = 199,
		Mach_max = 4,
		Nr_max = 25,
		OmViz_max = 0.52,
		Range_max = 80000,
		Reflection = 0.0329,
		X_back = -1.74,
		Y_back = -0.11,
		Z_back = 0,
		_file = "./CoreMods/aircraft/ChinaAssetPack/Entries/Loadouts/mils_ld10.lua",
		_origin = "China Asset Pack by Deka Ironwork Simulations and Eagle Dynamics",
		autopilot = {
			J_Angle_K = 0.20943951023932,
			J_Angle_W = 3.5,
			J_Diff_K = 0.4,
			J_FinAngle_K = 0.31415926535898,
			J_Int_K = 0,
			J_Power_K = 1.2,
			K = 100,
			Kg = 6,
			Ki = 0,
			delay = 1,
			finsLimit = 0.22,
			useJumpByDefault = 1
		},
		category = 2,
		class_name = "wAmmunitionSelfHoming",
		controller = {
			march_start = 0.021
		},
		exhaust = { 1, 1, 1, 0.5 },
		fm = {
			A = 0.36,
			I = 239.46333333333,
			Kw_x = 0.001,
			L = 3.8,
			Ma = 0.3,
			Ma_x = 0.001,
			Mw = 1.116,
			Sw = 0.85,
			caliber = 0.203,
			cx_coeff = { 1, 0.4, 1.1, 0.5, 1.4 },
			dCydA = { 0.07, 0.036 },
			finsTau = 0.1,
			mass = 199,
			maxAoa = 0.28,
			wind_sigma = 0,
			wind_time = 0
		},
		fuze_proximity = {
			arm_delay = 10,
			ignore_inp_armed = 1,
			radius = 10
		},
		march = {
			boost_factor = 0,
			boost_time = 0,
			custom_smoke_dissipation_factor = 0.2,
			fuel_mass = 70,
			impulse = 205,
			nozzle_orientationXYZ = { { 0, 0, 0 } },
			nozzle_position = { { -1.74, -0.11, 0 } },
			smoke_color = { 0.6, 0.6, 0.6 },
			smoke_transparency = 0.8,
			tail_width = 0.3,
			work_time = 10
		},
		mass = 199,
		model = "ld10",
		name = "LD-10",
		radio_seeker = {
			FOV = 0.087266462599716,
			aim_y_offset = 2,
			blind_ctrl_dist = 1000,
			blind_rad_val = 0.2,
			calc_aim_dist = 500000,
			err_correct_time = 1,
			err_val = 0.001,
			keep_aim_time = 8,
			min_sens_rad_val = 0.00025,
			op_time = 150,
			pos_memory_time = 60,
			sens_far_dist = 60000,
			sens_near_dist = 200
		},
		scheme = "schemes/missiles/anti_radiation_missile2.sch",
		shape_table_data = { {
				file = "ld10",
				fire = { 0, 1 },
				index = "Redacted",
				life = 1,
				name = "LD-10",
				username = "LD-10"
			} },
		sigma = { 10, 10, 10 },
		simple_gyrostab_seeker = {
			omega_max = 0.13962634015955
		},
		start_helper = {
			delay = 0.2,
			max_height = 400,
			max_vel = 200,
			power = 0.02,
			time = 2,
			use_local_coord = 0,
			vh_logic_or = 1
		},
		t_acc = 3,
		t_b = 0,
		t_marsh = 5,
		user_name = "LD-10",
		v_mid = 900,
		v_min = 140,
		warhead = {
			caliber = 90,
			concrete_factors = { 1, 1, 1 },
			concrete_obj_factor = 1,
			cumulative_factor = 2,
			cumulative_thickness = 0,
			expl_mass = 20,
			fantom = 0,
			mass = 20,
			obj_factors = { 2, 1 },
			other_factors = { 1, 1, 1 },
			piercing_mass = 0
		},
		warhead_air = {
			caliber = 90,
			concrete_factors = { 1, 1, 1 },
			concrete_obj_factor = 1,
			cumulative_factor = 2,
			cumulative_thickness = 0,
			expl_mass = 20,
			fantom = 0,
			mass = 20,
			obj_factors = { 2, 1 },
			other_factors = { 1, 1, 1 },
			piercing_mass = 0
		},
		wsTypeOfWeapon = { 4, 4, 8, "Redacted" }
	},
	sounderName = "Weapons/Missile",
	type_name = "missile",
	ws_type = { 4, 4, 8, "Redacted" }
}