_G["weapons_table"]["weapons"]["nurs"]["SNEB_TYPE251_H1"] = {
	_file = "./CoreMods/aircraft/AircraftWeaponPack/rockets.lua",
	_origin = "AircraftWeaponPack",
	_unique_resource_name = "weapons.nurs.SNEB_TYPE251_H1",
	client = {
		_file = "./CoreMods/aircraft/AircraftWeaponPack/rockets.lua",
		_origin = "AircraftWeaponPack",
		category = 3,
		class_name = "wAmmunitionNURS",
		control = {},
		displayName = "68 mm SNEB Type 251 H1 HE",
		engine = {
			boost_factor = 1,
			boost_tail = 1,
			boost_time = 0,
			fuel_mass = 1.485,
			impulse = 230,
			induced_RPS = 30,
			length = 0.608,
			nozzle_orientationXYZ = { { 0, 0, 0 } },
			nozzle_position = { { -0.43, 0, 0 } },
			smoke_color = { 0.6, 0.6, 0.6 },
			smoke_transparency = 0.5,
			tail_width = 0.068,
			total_mass = 3.26,
			work_tail = 1,
			work_time = 0.8
		},
		fm = {
			I = 0.24982053,
			L = 0.834,
			Ma = 0.23322,
			Mw = 2.177036,
			caliber = 0.068,
			cx_coeff = { 0.4, 0.89, 0.78, 0.22, 1.83 },
			freq = 30,
			mass = 4.31,
			rail_open = false,
			shapeName = "sneb68_eap",
			wind_sigma = 5,
			wind_time = 120
		},
		launcher = {
			server = 0
		},
		model = "sneb68_eap",
		name = "SNEB_TYPE251_H1",
		properties = {
			dist_max = 4000,
			dist_min = 500
		},
		scheme = "schemes/rockets/nurs-standard.sch",
		shape_table_data = { {
				file = "sneb68_eap2",
				index = "Redacted",
				name = "SNEB_TYPE251_H1"
			} },
		user_name = "SNEB Type 251 H1 HE",
		warhead = {
			caliber = 68,
			concrete_factors = { 1.33, 1.33, 0.1 },
			concrete_obj_factor = 1.33,
			cumulative_factor = 1.33,
			cumulative_thickness = 0,
			expl_mass = 0.5235,
			fantom = 1,
			length = 0.226,
			mass = 1.05,
			obj_factors = { 1.33, 1.33 },
			other_factors = { 1.33, 1.33, 1.33 },
			piercing_mass = 0.21
		},
		wsTypeOfWeapon = { 4, 7, 33, "Redacted" }
	},
	cx_pil = 1.414105917984e-05,
	display_name = "SNEB Type 251 H1 HE",
	dist_max = 4000,
	dist_min = 500,
	mass = 4.31,
	model = "sneb68_eap",
	name = "SNEB_TYPE251_H1",
	server = {
		_file = "./CoreMods/aircraft/AircraftWeaponPack/rockets.lua",
		_origin = "AircraftWeaponPack",
		category = 3,
		class_name = "wAmmunitionNURS",
		control = {},
		displayName = "68 mm SNEB Type 251 H1 HE",
		engine = {
			boost_factor = 1,
			boost_tail = 1,
			boost_time = 0,
			fuel_mass = 1.485,
			impulse = 230,
			induced_RPS = 30,
			length = 0.608,
			nozzle_orientationXYZ = { { 0, 0, 0 } },
			nozzle_position = { { -0.43, 0, 0 } },
			smoke_color = { 0.6, 0.6, 0.6 },
			smoke_transparency = 0.5,
			tail_width = 0.068,
			total_mass = 3.26,
			work_tail = 1,
			work_time = 0.8
		},
		fm = {
			I = 0.24982053,
			L = 0.834,
			Ma = 0.23322,
			Mw = 2.177036,
			caliber = 0.068,
			cx_coeff = { 0.4, 0.89, 0.78, 0.22, 1.83 },
			freq = 30,
			mass = 4.31,
			rail_open = false,
			shapeName = "sneb68_eap",
			wind_sigma = 5,
			wind_time = 120
		},
		launcher = {
			server = 1
		},
		model = "sneb68_eap",
		name = "SNEB_TYPE251_H1",
		properties = {
			dist_max = 4000,
			dist_min = 500
		},
		scheme = "schemes/rockets/nurs-standard.sch",
		shape_table_data = { {
				file = "sneb68_eap2",
				index = "Redacted",
				name = "SNEB_TYPE251_H1"
			} },
		user_name = "SNEB Type 251 H1 HE",
		warhead = {
			caliber = 68,
			concrete_factors = { 1.33, 1.33, 0.1 },
			concrete_obj_factor = 1.33,
			cumulative_factor = 1.33,
			cumulative_thickness = 0,
			expl_mass = 0.5235,
			fantom = 0,
			length = 0.226,
			mass = 1.05,
			obj_factors = { 1.33, 1.33 },
			other_factors = { 1.33, 1.33, 1.33 },
			piercing_mass = 0.21
		},
		wsTypeOfWeapon = { 4, 7, 33, "Redacted" }
	},
	sight_data = {
		engine = {
			fuel_mass = 1.485,
			impulse = 230,
			work_time = 0.8
		},
		fm = {
			I = 0.24982053,
			L = 0.834,
			Ma = 0.23322,
			Mw = 2.177036,
			caliber = 0.068,
			cx_coeff = { 0.4, 0.89, 0.78, 0.22, 1.83 },
			freq = 30,
			mass = 4.31,
			rail_open = false,
			shapeName = "sneb68_eap",
			wind_sigma = 5,
			wind_time = 120
		}
	},
	sounderName = "Weapons/Rocket",
	type_name = "rocket",
	ws_type = { 4, 7, 33, "Redacted" }
}